...
class MenuFrameC4 extends JFrame implements ActionListener {
	public JButton  menub, choob, boarb, cartb, snackb, breab, drinkb_c, icecb, candb, etccb, 토레타
	,두유,식혜,헬로파, 헬로분, 결명자, 옥수수, left, right;
	...
	public MenuFrameC4()
	  {
	   ...
	   토레타 = new JButton(new ImageIcon("img/u-25.png"));
	   두유 = new JButton(new ImageIcon("img/u-26.png"));
	   식혜 = new JButton(new ImageIcon("img/u-27.png"));
	   헬로파 = new JButton(new ImageIcon("img/u-28.png"));
	   헬로분 = new JButton(new ImageIcon("img/u-29.png"));
	   결명자 = new JButton(new ImageIcon("img/u-30.png"));
	   옥수수 = new JButton(new ImageIcon("img/u-31.png"));
	   left = new JButton(new ImageIcon("img/a-50.png"));
	   page = new JLabel("4/4");page.setFont(f1);
	 ...
	 //  ImageIcon logo = new ImageIcon("logo.jpg");
	  ...
	  // JPanel mm = new JPanel();
	   ...
	   c.add(헬로분); c.add(결명자); c.add(옥수수); c.add(left);
	   c.add(page); 
	   ...
	   setResizable(false);
		...
		public void actionPerformed(ActionEvent e){
			...
			if(e.getSource().equals(토레타))
			{
				try {
	                ...
	                st.execute("use menu;"); 
	                result = st.executeQuery("select name from allmenu where name = '서울우유(딸기)';");
	                ...
	                while (result.next()){
	                   ...
	                   int reply = JOptionPane.showConfirmDialog(this, str);
	                    if(reply == JOptionPane.YES_OPTION){
	                    	...
	                    	st2.execute("update allmenu set product=product+1 where name='서울우유(딸기)';");
	                    	st2.execute("insert into cart values('서울우유(딸기)','800')");
	                    }
	                }
				} catch (Exception e1) {
					e1.printStackTrace();
			...
			else if (e.getSource().equals(두유))
		{
				try {
	                ...
	                st = con.createStatement();
	                st.execute("use menu;"); 
	                result = st.executeQuery("select name from allmenu where name = '삼다수';");
	                ...
	                while (result.next()){
	                   ...
	                   int reply = JOptionPane.showConfirmDialog(this, str);
	                    if(reply == JOptionPane.YES_OPTION){
	                    	 java.sql.Statement st2 = null;
	                         st2 = con.createStatement();
	                    	st2.execute("update allmenu set product=product+1 where name='삼다수';");
	                    	st2.execute("insert into cart values('삼다수','1000')");
	                    }
	                }
				} catch (Exception e1) {
					e1.printStackTrace();
		...
		else if(e.getSource().equals(식혜))
		{
			try {
                ...
                st = con.createStatement();
                st.execute("use menu;"); 
                result = st.executeQuery("select name from allmenu where name = '순창샘물';");
                ...
                while (result.next()){
                   ...
                   int reply = JOptionPane.showConfirmDialog(this, str);
                    if(reply == JOptionPane.YES_OPTION){
                    	 java.sql.Statement st2 = null;
                         st2 = con.createStatement();
                    	st2.execute("update allmenu set product=product+1 where name='순창샘물';");
                    	st2.execute("insert into cart values('순창샘물','1000')");
                    }
                }
			} catch (Exception e1) {
				e1.printStackTrace();
		...
		else if(e.getSource().equals(헬로파))
		{
			try {
                ...
                st = con.createStatement();
                st.execute("use menu;"); 
                result = st.executeQuery("select name from allmenu where name = '샤인온더비치';");
                ...
                while (result.next()){
                   ...
                   int reply = JOptionPane.showConfirmDialog(this, str);
                    if(reply == JOptionPane.YES_OPTION){
                    	 java.sql.Statement st2 = null;
                         st2 = con.createStatement();
                    	st2.execute("update allmenu set product=product+1 where name='샤인온더비치';");
                    	st2.execute("insert into cart values('샤인온더비치','1200')");
                    }
                }
			} catch (Exception e1) {
				e1.printStackTrace();
		...
		else if(e.getSource().equals(헬로분))
		{
			try {
                ...
                st = con.createStatement();
                st.execute("use menu;"); 
                result = st.executeQuery("select name from allmenu where name = '블루하와이';");
                ...
                while (result.next()){
                   ...
                   int reply = JOptionPane.showConfirmDialog(this, str);
                    if(reply == JOptionPane.YES_OPTION){
                    	 java.sql.Statement st2 = null;
                         st2 = con.createStatement();
                    	st2.execute("update allmenu set product=product+1 where name='블루하와이';");
                    	st2.execute("insert into cart values('블루하와이','1200')");
                    }
                }
			} catch (Exception e1) {
				e1.printStackTrace();
		...
		else if(e.getSource().equals(결명자))
		{
			try {
                ...
                st = con.createStatement();
                st.execute("use menu;"); 
                result = st.executeQuery("select name from allmenu where name = '모히또';");
                ...
                while (result.next()){
                   ...
                   int reply = JOptionPane.showConfirmDialog(this, str);
                    if(reply == JOptionPane.YES_OPTION){
                    	 java.sql.Statement st2 = null;
                         st2 = con.createStatement();
                    	st2.execute("update allmenu set product=product+1 where name='모히또';");
                    	st2.execute("insert into cart values('모히또','1200')");
                    }
                }
			} catch (Exception e1) {
				e1.printStackTrace();
		...
		else if(e.getSource().equals(옥수수))
		{
			try {
                ...
                st = con.createStatement();
                st.execute("use menu;"); 
                result = st.executeQuery("select name from allmenu where name = '파워에이드';");
                ...
                while (result.next()){
                   ...
                   int reply = JOptionPane.showConfirmDialog(this, str);
                    if(reply == JOptionPane.YES_OPTION){
                    	 java.sql.Statement st2 = null;
                         st2 = con.createStatement();
                    	st2.execute("update allmenu set product=product+1 where name='파워에이드';");
                    	st2.execute("insert into cart values('파워에이드','1500')");
                    }
                }
			} catch (Exception e1) {
				e1.printStackTrace();
